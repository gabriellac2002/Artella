<!DOCTYPE html>
<html lang="pt-br">

<head>
  <meta charset="UTF-8" />
  <meta http-equiv="X-UA-Compatible" content="IE=edge" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <!-- <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script> -->

  <link rel="stylesheet" href="../../../public/fontawesome/css/all.css" />
  <link rel="stylesheet" href="../../../public/css/footerStyle.css" />
  <link rel="stylesheet" href="../../../public/css/styles.css" />
</head>

<body class="bg-universal">
  <footer class="text-footer mt-4">
    <div class="footer-align-area icons-footer">
      <a href="#" class="mx-4 text-reset">
        <i class="fab fa-facebook-f"></i>
      </a>

      <a href="#" class="mx-4 text-reset">
        <i class="fab fa-instagram"></i>
      </a>

      <a href="#" class="mx-4 text-reset">
        <i class="fab fa-linkedin"></i>
      </a>
    </div>

    <div class="container footer-align-area mt-5">
      <div class="row mt-3">
        <div class="col-md-3 col-lg-4 col-xl-3 mx-auto mb-4">
          <h6 class="title-footer mb-2 mb-lg-4 mt-2 mt-lg-0">Artella</h6>

          <q class="text-footer">
            A finalidade da arte é dar corpo à essência secreta das coisas,
            não é copiar a sua aparência. (Aristóteles)
          </q>
        </div>

        <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mb-4">
          <h6 class="title-footer mb-2 mb-lg-4 mt-2 mt-lg-0">Produtos</h6>
          <p>
            <a href="#" class="text-reset text-footer">Obras</a>
          </p>
          <p>
            <a href="#" class="text-reset text-footer">NFTs</a>
          </p>
          <p>
            <a href="#" class="text-reset text-footer">Artistas</a>
          </p>
          <p>
            <a href="#" class="text-reset text-footer">Exposições</a>
          </p>
        </div>
        <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mb-4">
          <h6 class="title-footer mb-2 mb-lg-4 mt-2 mt-lg-0">
            Sobre Artella
          </h6>
          <p>
            <a href="#" class="text-reset text-footer">Quem somos?</a>
          </p>
        </div>
        <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mb-md-0 mb-4">
          <h6 class="title-footer mb-2 mb-lg-4 mt-2 mt-lg-0">Contato</h6>
          <p>
            <a href="" class="text-reset text-footer">
              <i class="fa-brands fa-whatsapp mx-lg-3 mx-1"></i>
              +55 (32) 99999-9999
            </a>
          </p>
          <p>
            <a href="" class="text-reset text-footer">
              <i class="fas fa-envelope mx-lg-3 mx-1"></i>
              Artella@email.com
            </a>
          </p>
          <p>
            <a href="" class="text-reset text-footer">
              <i class="fas fa-phone mx-lg-3 mx-1"></i>
              Fale conosco
            </a>
          </p>
        </div>
      </div>
    </div>
    <div class="copyright-area text-dark" style="font-size: 12px">
      <i class="fa fa-copyright"></i> 2022 Artella.com
    </div>
  </footer>
</body>

</html>